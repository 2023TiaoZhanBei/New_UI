...
from . import config_values as cfg
...
class Identify(QObject):
    ...
    def run(self):
        ...
        while opened:
            ...
            if len(lmList) != 0:
                ...
                print(fingers)
                cv2.rectangle(image, (cfg.frameR, cfg.frameR), (cfg.wCam - cfg.frameR, cfg.hCam - cfg.frameR),
                              (255, 0, 255), 2)

                if cfg.NOW_MODE == cfg.MOVE_AND_CLICK:
                    image = self.MOVE_AND_CLICK_FUNC(fingers, cfg.hCam, image, cfg.wCam, x1, y1)
                ...
                if fingers == [0, 1, 1, 1, 1]:
                    ...
                    if time.time() - cfg.bef_selecting > 0.8:
                        cfg.NOW_MODE, cfg.NOW_MODE_COLOR = self.select_mode(image, lmList, cfg.offset, x2, y2)
                        selecting = True
                else:
                    cfg.bef_selecting = 0
                    selecting = False
    ...
    def reaction(self, device, h_t, hiddem_dim, in_dim, last_gesture, model, movement, num_layers, prin_time):
        ...
        # 对每个动作设置单独的置信度阈值
        cfd = {'点击': 0.90, '平移': 0.90, '缩放': 0.99, '抓取': 0.985, '旋转': 0.99, '无': 0, '截图': 0.95,
               '放大': 0.9}
