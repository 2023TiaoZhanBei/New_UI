...
class FBall(QDialog):
    ...
    def expand(self, offset_x, offset_y):
        ...
        # 获取当前位置
        self.anim.setEndValue(QPoint(offset_x - (self.ball_size // 2), offset_y - (self.ball_size // 2)))
...
class MainLandBall(FBall):
    ...
    def expand(self, move_length, pre_angle=40):
        ...
        x0, y0 = self.pos().x() + (self.ball_size // 2), self.pos().y() + (self.ball_size // 2)
        self.is_expanded = True

        for i, (angle_i, color, color_deep) in enumerate(
                zip(range(0 + self.offset, int(pre_angle * 5 + 1 + self.offset), pre_angle), self.color_set,
                    self.color_set_deep)):
